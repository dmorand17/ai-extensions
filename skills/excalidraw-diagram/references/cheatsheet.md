...
| `get_element` | Get element by ID | `id` |
...
| Tool | Description | Required params |
|------|-------------|-----------------|
| `describe_scene` | Structured text: IDs, types, positions, labels, connections, bounding box | (none) |
| `get_canvas_screenshot` | PNG of current canvas for visual verification | (optional) `background` |
...
| `export_to_image` | Export PNG/SVG (needs browser) | `format` ("png"\|"svg"), (optional) `filePath`, `background` |
| `export_to_excalidraw_url` | Shareable excalidraw.com URL | (none) |
...
| Tool | Description | Required params |
|------|-------------|-----------------|
| `read_diagram_guide` | Design best practices: colors, sizing, layout, anti-patterns | (none) |
...
- **MCP labels**: `"text": "My Label"` on shapes (auto-converts). `startElementId`/`endElementId` on arrows.
- **REST labels**: `"label": {"text": "My Label"}` on shapes. `"start": {"id": "..."}` / `"end": {"id": "..."}` on arrows.
- `fontFamily` must be a string (e.g. `"1"`) or omitted — never a number.
- `points` accepts `[[x,y]]` tuples or `[{x,y}]` objects.
- Curved arrows: `"roundness": {"type": 2}` with 3+ points.
...
| `POST` | `/api/elements/from-mermaid` | Mermaid conversion |
...
| `POST` | `/api/export/image/result` | Frontend posts result back |
...
| `POST` | `/api/viewport/result` | Frontend posts result back |
...
All accept `--url <canvasUrl>` (defaults to `EXPRESS_SERVER_URL`).
